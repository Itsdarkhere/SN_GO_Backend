...
const (
	...
	RoutePathAddToCollection = "/api/v0/add-to-collection"
	// IMX SUPERNOVAS
...
func (fes *APIServer) NewRouter() *muxtrace.Router {
	var FrontendRoutes = []Route{
		...
		},
		{
			"CreateCollection",
			[]string{"POST", "OPTIONS"},
			RoutePathCreateCollection,
			fes.CreateCollection,
			PublicAccess,
		},
		{
			"SortCollection",
		...
		},
		{
			...
			[]string{"POST", "OPTIONS"},
			RoutePathGetAllUserCollections,
			fes.GetAllUserCollections,
			PublicAccess,
		},
		{
			"GetCollectionInfo",
