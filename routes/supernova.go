...
func (fes *APIServer) SortCollection(ww http.ResponseWriter, req *http.Request) {
	...
	coin_royalty_basis_points, num_nft_copies_for_sale, num_nft_copies_burned, pg_posts.extra_data, collection, 
...
func (fes *APIServer) GetTrendingAuctions(ww http.ResponseWriter, req *http.Request) {
	...
	AND num_nft_copies != num_nft_copies_burned
	GROUP BY post_hash
	ORDER BY MAX(bid_amount_nanos) desc LIMIT 9`, timeUnix))
	if err != nil {
...
func (fes *APIServer) GetCreatedNFTs(ww http.ResponseWriter, req *http.Request) {
	decoder := json.NewDecoder(io.LimitReader(req.Body, MaxRequestBodySizeBytes))
	requestData := GetProfileNFTsRequest{}
	if err := decoder.Decode(&requestData); err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetCreatedNFTs: Error parsing request body: %v", err))
		return
	}

	var readerPublicKeyBytes []byte
	var err error
	if requestData.ReaderPublicKeyBytes != "" {
		readerPublicKeyBytes, _, err = lib.Base58CheckDecode(requestData.ReaderPublicKeyBytes)
		if err != nil {
			_AddBadRequestError(ww, fmt.Sprintf("GetCreatedNFTs: Problem decoding ReaderPublicKeyBytes: %v", err))
			return
		}
	}

	if requestData.Username == "" {
		_AddBadRequestError(ww, fmt.Sprintf("GetCreatedNFTs: No Username sent in request"))
		return
	}
	...
	dbPool, err := CustomConnect()
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetCreatedNFTs: Error getting pool: %v", err))
		return
	}
	// get connection to pool
	conn, err := dbPool.Acquire(context.Background())
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetCreatedNFTs: Error cant connect to database: %v", err))
		conn.Release()
		return
	}
	...
	pinned, nft, num_nft_copies, unlockable, creator_royalty_basis_points,
	coin_royalty_basis_points, num_nft_copies_for_sale, num_nft_copies_burned, extra_data FROM pg_posts
	INNER JOIN pg_profiles ON 
	poster_public_key = public_key WHERE extra_data->>'isEthereumNFT' = 'dHJ1ZQ==' OR nft = true AND username = '%v' 
	ORDER BY timestamp desc OFFSET %v LIMIT 20`, requestData.Username, offset))
	if err != nil {
		_AddBadRequestError(ww, fmt.Sprintf("GetCreatedNFTs: Error query failed: %v", err))
		return
	} else {
        ...
        for rows.Next() {
            ...
            rows.Scan(&post.LikeCount, &post.DiamondCount, &post.CommentCount, &post.PostHashHex, 
				...
				if rows.Err() != nil {
					// if any error occurred while reading rows.
					_AddBadRequestError(ww, fmt.Sprintf("GetCreatedNFTs: Error scanning to struct: %v", err))
					return
				}
			if post.PostExtraData["name"] != "" {
				post.PostExtraData["name"] = base64Decode(post.PostExtraData["name"])
			...
			utxoView, err := fes.backendServer.GetMempool().GetAugmentedUniversalView()
			if err != nil {
				_AddBadRequestError(ww, fmt.Sprintf("GetCreatedNFTs: Error getting utxoView: %v", err))
				return
			}
			...
			postHash, err := GetPostHashFromPostHashHex(post.PostHashHex)
			if err != nil {
				_AddBadRequestError(ww, fmt.Sprintf("GetCreatedNFTs: %v", err))
				return
			}
			// Fetch the postEntry requested.
			postEntry := utxoView.GetPostEntryForPostHash(postHash)
			if postEntry == nil {
				_AddBadRequestError(ww, fmt.Sprintf("GetCreatedNFTs: Could not fetch postEntry"))
				return
			}
			// Get info regarding the readers interactions with the post
			post.PostEntryReaderState = utxoView.GetPostEntryReaderState(readerPublicKeyBytes, postEntry)
		...
		if err = json.NewEncoder(ww).Encode(resp); err != nil {
			_AddInternalServerError(ww, fmt.Sprintf("GetCreatedNFTs: Problem serializing object to JSON: %v", err))
			return
		}
		// Just to make sure
		conn.Release()
...
func (fes *APIServer) GetNFTsByCategory(ww http.ResponseWriter, req *http.Request) {
	...
	if requestData.ReaderPublicKeyBase58Check != "" {
		readerPublicKeyBytes, _, err = lib.Base58CheckDecode(requestData.ReaderPublicKeyBase58Check)
		if err != nil {
			_AddBadRequestError(ww, fmt.Sprintf("GetNFTsByCategory: Problem decoding reader public key: %v", err))
			return
		}
	}
